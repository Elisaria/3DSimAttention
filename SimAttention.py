...
class Down_Projection(layers.Layer):
...
class Random_Projection(layers.Layer):
...
class SimAttention(layers.Layer):
    ...
    def __init__(self, d_l, d_h, k_h, k, window_size=9, **kwargs):
        super().__init__(**kwargs)
        self.reshape = layers.Reshape((-1, window_size, d_l))
        ...
        self.k_h = k_h
        self.DOWN_PROJECTION_1 = layers.Dense(d_l, activation='relu')
